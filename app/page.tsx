    ...
    <div className="min-h-screen bg-slate-950 px-6 py-12 text-white">
      <main className="mx-auto flex w-full max-w-4xl flex-col gap-8 rounded-3xl border border-white/10 bg-slate-900/60 p-10">
        <header className="space-y-3">
          ...
          <h1 className="text-4xl font-semibold">REST API Calls: Image Upload + Captions</h1>
          <p className="text-slate-300">
            Upload an image, run the full caption pipeline (presigned URL, upload, register, generate),
            and then vote on captions.
          </p>
        </header>
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <p className="mt-2 text-sm text-slate-300">Sign in on the protected page.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <h2 className="text-lg font-semibold text-white">Step 2</h2>
            <p className="mt-2 text-sm text-slate-300">Upload an image and generate captions via API.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <h2 className="text-lg font-semibold text-white">Step 3</h2>
            <p className="mt-2 text-sm text-slate-300">Vote on shown captions and validate DB inserts.</p>
          </div>
        </section>
